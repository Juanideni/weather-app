...
function TodayWeather(props){
   ...
   let firstDay
  let getTime = new Date(infoExtra.time);
  let idDate
  useEffect(() => {
    if (props.id !== 0) {
   ...
   firstDay = currentDay[0]
    ...
    return(
     ...
     <div className="div-day">
        ...
        <h3>
          Min temp: {Number(firstDay.min_temp).toFixed(0)} °C{" "}
        </h3>
        <h3>
          Max temp: {Number(firstDay.max_temp).toFixed(0)} °C
        </h3>
        <h3>
          Air pressure: {Number(firstDay.air_pressure).toFixed(0)}{" "}
          mbar
        </h3>
        <h3>
          Humidity: {Number(firstDay.humidity).toFixed(0)} %{" "}
        </h3>
        <h3>
        ...
        <h3>
          Visibility: {Number(firstDay.visibility).toFixed(0)}{" "}
          miles
        </h3>
      </h5>
      </div>
